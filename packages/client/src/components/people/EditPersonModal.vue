<template>
  <n-modal v-model:show="show" preset="card" title="Edit Person" style="width: 600px">
    <n-form
      ref="formRef"
      :model="formData"
      :rules="rules"
    ...
    >
      ...
      <n-form-item label="Favorite" path="isFavorited">
        <n-switch v-model:value="formData.isFavorited" />
        <span style="margin-left: 8px; color: #666;">Mark as favorite</span>
      </n-form-item>
    </n-form>
...
const formData = reactive<UpdatePersonRequest>({
  name: '',
  description: null,
  relationship: null,
  notableEvents: null,
...
watch(() => props.person, (newPerson) => {
  if (newPerson) {
    formData.name = newPerson.name
    formData.description = newPerson.description
    formData.relationship = newPerson.relationship
    formData.notableEvents = newPerson.notableEvents
    formData.url = newPerson.url
    formData.isFavorited = newPerson.isFavorited
  }
}, { immediate: true })
